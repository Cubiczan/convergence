...
**Adoption:** `src/convergence/mesh/rubric.py` grades that narrative with four named, deterministic criteria — `root_cause_named` (30), `why_links_resolved` (30), `grounding_sourced` (25), `high_confidence_contribution` (15). It flags what the synthesizer would otherwise hide: placeholder-padded why-chains (the `_synthesize_statement` fallback pads to three links with `PADDING_WHY_ANSWER` — a narrative resting on padding no longer reads as clean), unsourced grounding claims (the message distinguishes "no grounding checks recorded" from "grounding attempted, claim lacks source" — same 25-point penalty, different remediation), and narratives with no high-confidence contribution (distinguishing agents-uncertain-because-the-question-is-hard — turns name `what_would_change` — from incomplete analysis — nothing does).

**Enforcement (the verdict gates, it does not decorate):** a `REQUIRES_HUMAN_VERIFICATION` verdict means the same thing the CHP state machine's identically-named status means. `CHPOrchestrator` wired with a signed `AuditLedger` refuses lock promotion while a failing verdict governs the problem: `advance_to_provisional_lock` blocks PROVISIONAL_LOCK and `apply_validation` rejects a CONFIRM before mutation — `LOCKED`-with-failing-verdict is an invalid system state that cannot arise. The join key is the narrative's problem statement (by convention, `orchestrate()` is called with the case title). CHP-only flows without a graded narrative run without ceremony.

**Clearance paths (either unblocks promotion):** (1) a human acknowledgment — `record_narrative_ack` appends a `narrative_rubric_ack` event to the signed ledger with `confirmed_by` naming the human (four-eyes; the ack is not machine-authorable and clears only verdicts that came BEFORE it — a new failing verdict re-blocks); or (2) regenerate the narrative and re-grade — the latest `narrative_rubric` verdict wins.

**Ledger surface:** `narrative_rubric` and `narrative_rubric_ack` events carry machine-readable `status`/`score` in `inputs` and are verified by the same HMAC chain as the narrative itself (`ledger.verify()`). There is no dedicated dashboard UI yet — the signed ledger (`AuditLedger.read_all()`) is the read surface; a `GET /audit` API route on `src/convergence/api/` is the natural future surface when one is wanted.

**Join-key hazard (rename/mismatch):** the join key between narrative and case is the problem string (by convention, the case title). Renaming a case title after its narrative was graded — or grading under a different problem string than the title — makes promotion find NO matching verdict, and the gate silently opens. The orchestrator makes that shape loud, not silent: promotion (PROVISIONAL_LOCK advancement or LOCKED-confirm) with no matching verdict **while other verdicts exist in the ledger** logs an `AUDIT RISK` warning naming the case, the orphan risk, and where to look (ledger / `GET /audit`). A genuine CHP-only flow (no narratives graded anywhere) stays silent — the warning fires only on the mismatch shape, and it is an audit signal, not a block.

**Ledger wiring is required for enforcement:** a `CHPOrchestrator` constructed **without** a signed `AuditLedger` logs a loud warning at construction — rubric enforcement is disabled for that orchestrator and lock promotion is ungated. Production deployments must wire a ledger for the row-35 governance to apply; the warning exists so silently-ungoverned operation is discoverable in logs.

**Governance spec (explicit):** CHP-only `LOCKED` decisions (no graded narrative) are an acceptable current state — enforcement binds only where a narrative exists; the `GET /audit` route is the migration surface when narrative coverage becomes universal. The `narrative_rubric_ack` owner is the **deal manager** for the case (not a compliance role — the deal manager owns the CoA decision the narrative supports; compliance owns the broader ledger audit). Discovery of a blocked promotion today is via the signed ledger (`AuditLedger.read_all()` / `verify()`); the `GET /audit` route is the planned self-serve surface.
